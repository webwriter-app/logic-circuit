...
| `zoom` (`zoom`) | `number` | Current zoom level of the workspace.
@description Controls the scaling of the workspace. 1.0 = 100%, 2.0 = 200%, etc.
@default 1
@minimum 0.1
@maximum 5.0 | `1` | ✗ |
| `simulate` (`simulate`) | `boolean` | Whether the circuit simulation is currently active.
@description When true, the circuit will automatically calculate and display logic values.
@default true | `true` | ✗ |
| `simulationDelay` (`simulationDelay`) | `number` | Delay in milliseconds between simulation steps.
...
@minimum 0
@maximum 5000
@attribute simulationdelay | `500` | ✓ |
| `allowSimulation` (`allowSimulation`) | `number` | Whether simulation controls are enabled.
...
@minimum 0
@maximum 1
@attribute allowsimulation | `1` | ✓ |
| `notGateAllowed` (`notGateAllowed`) | `number` | Maximum number of NOT gates allowed in the circuit.
...
@default -1
@minimum -1
@attribute notgateallowed | `-1` | ✓ |
| `andGateAllowed` (`andGateAllowed`) | `number` | Maximum number of AND gates allowed in the circuit.
...
@default -1
@minimum -1
@attribute andgateallowed | `-1` | ✓ |
| `orGateAllowed` (`orGateAllowed`) | `number` | Maximum number of OR gates allowed in the circuit.
...
@default -1
@minimum -1
@attribute orgateallowed | `-1` | ✓ |
| `nandGateAllowed` (`nandGateAllowed`) | `number` | Maximum number of NAND gates allowed in the circuit.
...
@default -1
@minimum -1
@attribute nandgateallowed | `-1` | ✓ |
| `norGateAllowed` (`norGateAllowed`) | `number` | Maximum number of NOR gates allowed in the circuit.
...
@default -1
@minimum -1
@attribute norgateallowed | `-1` | ✓ |
| `xnorGateAllowed` (`xnorGateAllowed`) | `number` | Maximum number of XNOR gates allowed in the circuit.
...
@default -1
@minimum -1
@attribute xnorgateallowed | `-1` | ✓ |
| `xorGateAllowed` (`xorGateAllowed`) | `number` | Maximum number of XOR gates allowed in the circuit.
@description See notGateAllowed for detailed value meanings. XOR gates output true when inputs differ (exclusive or).
@default -1
@minimum -1
@attribute xorgateallowed | `-1` | ✓ |
| `splitterAllowed` (`splitterAllowed`) | `number` | Maximum number of splitter gates allowed in the circuit.
@description See notGateAllowed for detailed value meanings. Splitter gates duplicate input to two separate outputs.
@default -1
@minimum -1
@attribute splitterallowed | `-1` | ✓ |
...
| :--: | :---------: | :-------: |
| `toggleInstructions` | A WebWriter widget for creating and simulating digital logic circuits.
...
| `handleDrop` | Handles drop events when gates are dropped on the workspace or trash can.
...
@fires circuit-changed - Dispatched when the circuit structure changes | `event: DragEvent`
| `handleDropTrashCan` | Handles deletion of gates when dropped on the trash can.
@description Finds and removes the gate that was dropped on the trash area.
@param {DragEvent} event - The drop event containing the gate ID to delete | `event: DragEvent`
| `handleInputChange` | Handles changes to input fields in the options panel.
...
@param {string} propertyName - The name of the property to update (e.g., 'andGateAllowed', 'simulationDelay') | `event: Event`, `propertyName: string`
...
| simulation-state-changed | Custom event dispatched when simulation state changes |
